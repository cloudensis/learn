...
- 答えの根拠になった公式資料のページがあれば、そのURLを添えてください`}
				...
				<h2>しあげに、ロゴ画像を足す</h2>
				<p>
					最後に、公開したゲームにロゴ画像を足して、公開し直してみます。
					公開し直すやり方の練習です。
				</p>
				<p>
					まず、<code>my-game-backup</code> を取り直してください。
					次に、この本が用意したロゴ画像をダウンロードします。
				</p>
				<p>
					<a href="/books/first-steps/Logo.svg" download="Logo.svg">
						ロゴ画像をダウンロードする
					</a>
				</p>
				<p>
					ダウンロードしたファイルは、
					<strong>名前を変えずに</strong>、そのまま <code>my-game</code>{" "}
					フォルダへ移してください。 そのうえで、新しい会話で次の文を送ります。
					丸かっこには、いまの <code>index.html</code> の全文を貼り付けます。
				</p>
				<AskAi title="新しい会話で送ってください">
					{`ブラウザで遊べるゲームの上部に、ロゴの画像を表示したいです。

- ロゴの画像は logo.svg という名前で、index.html と同じフォルダに置いてあります
- index.html のどこに何を足せばロゴが表示されるかを、変更後の index.html の全文で出してください
- ロゴは、ゲームの横幅からはみ出さない大きさで表示してください
- ゲームの内容は変えないでください
...
（全文）`}
				...
				<p>
					<code>index.html</code> を、返ってきた全文で上書きして保存します。
				</p>
				<p>
				...
				<Troubleshoot
					...
					items={[
						...
						{
							symptom: "ダウンロードしたロゴ画像が、どこにあるか分からない",
							action:
								"多くのブラウザでは、パソコンの「ダウンロード」フォルダに保存されます。そこから、名前を変えずに my-game フォルダへ移してください。",
						},
						{
